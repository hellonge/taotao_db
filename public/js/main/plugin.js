...
function cxInfoOfDateTime(solar, lunar) {
    ...
    var cindex1 = cindex - 6;
    if (cindex1 < 0) {
        cindex1 = cindex1 + 12;
    }

    var urls = "/view/name?name=黄历现代文";
    //urls+='&'+encodeURI(oName.value)+'&gender=man';
    urls += '&ri=' + ANIMAL[cindex1] + '&chong=' + ANIMAL[cindex] + '&sha=' + CompassNames[sindex].replace("正", "") + '&xiangchong1=' + Zhi[cindex1] + '&xiangchong2=' + Zhi[cindex];
    document.getElementById('huanglixiandaiwen').href = urls;

    try {
        return ANIMAL[cindex1] + "日冲" + ANIMAL[cindex] + "煞" + CompassNames[sindex].replace("正", "");
    }
    catch (e) {
        return "";
    }
...
//document.getElementById('huanglixiandaiwen').href="/view/name?name=黄历现代文&&"+'ri=猪头';
